...
**Status:** v0.4 — **planning complete 2026-07-18** (specs 000–004 written; OI-1/2/3/4 all resolved); tooling de-risked 2026-07-17; scope decisions 2026-07-14. Next: M1 implementation on owner go-ahead.
**Owner:** siddw143
...
- **OI-4** ~~Task source for the M3 proof eval.~~ **Resolved 2026-07-18:** **synthetic decay-stress tasks** (owner pick) — see [spec 004](004_proof_eval.md). Constraint established early → distractor sized past `num_ctx=4096` → late decision point → deterministic verifier; with a mandatory decay-gap control (short vs long variant) so the tasks are provably easy-when-remembered. Terminal-Bench/τ² ruled out on 4 GB.

## 9. Next specs

[`001_memory_bank.md`](001_memory_bank.md) (schema, tools, serialization — closes G3/G4/G5/G11) — **written 2026-07-14** · `002_two_phase_agent.md` (prompts, `override(system_message=…)` injection contract, sync model, shared-`num_ctx` budgets, N=4 — closes G1/G2/G6/G7/G8) · [`003_harness_adapter.md`](003_harness_adapter.md) (`create_agent` + custom middleware; `before_model` runs the memory step, `wrap_model_call` gates the one-shot `override(system_message)`; harness-agnostic `MemorySidecar` protocol; version pins — closes G10) — **written 2026-07-18** · [`004_proof_eval.md`](004_proof_eval.md) (synthetic decay-stress tasks, decay-gap control, paired baseline-vs-memory, token/latency metering — closes G9, OI-4) — **written 2026-07-18**. **All planning specs complete.** Specs 002/003 build directly on the [2026-07-17 de-risk memo](../docs/research/2026-07-17_tooling_derisk.md).
